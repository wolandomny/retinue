...
func buildAgentSystemPrompt(ws *workspace.Workspace, agent *standing.Agent) string {
	...
	fmt.Fprintf(&b, "Woland sends you messages when needed — you'll see them as injected text\n")
	fmt.Fprintf(&b, "in your session. Messages appear in this format:\n")
	fmt.Fprintf(&b, "    [Woland] a message or instruction from Woland\n")
	fmt.Fprintf(&b, "    [User] a message from the user, relayed by Woland\n\n")
	fmt.Fprintf(&b, "When you respond, your message is automatically sent to Woland (the hub).\n")
	fmt.Fprintf(&b, "You cannot message other agents directly — ask Woland to relay if needed.\n")
