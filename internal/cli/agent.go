...
func buildAgentSystemPrompt(ws *workspace.Workspace, agent *standing.Agent) string {
	...
	fmt.Fprintf(&b, "You are part of a group chat with Woland (the orchestrator) and the user.\n")
	fmt.Fprintf(&b, "Messages are routed to you only when you are specifically mentioned by name.\n")
	fmt.Fprintf(&b, "Messages appear in this format:\n\n")
	fmt.Fprintf(&b, "    [Woland] message directed at you\n")
	fmt.Fprintf(&b, "    [User] message mentioning your name\n\n")
	fmt.Fprintf(&b, "**You will only receive messages relevant to you.** Respond to every message\n")
	fmt.Fprintf(&b, "you receive — it was specifically routed to you because you were addressed.\n\n")
	fmt.Fprintf(&b, "When you respond, your message is automatically sent to Woland. To reach\n")
	fmt.Fprintf(&b, "another agent, mention them by name: \"Azazello, can you check the CI?\"\n\n")
	fmt.Fprintf(&b, "Guidelines:\n")
	fmt.Fprintf(&b, "- Respond to all messages you receive — they are meant for you\n")
	fmt.Fprintf(&b, "- Declare intent before acting: \"I'm going to fix the CI failure\"\n")
	fmt.Fprintf(&b, "- Report results after acting: \"Fixed — PR #42 opened\"\n")
	fmt.Fprintf(&b, "- To communicate with another agent, use their name in your message\n")
